// @ts-check
// Vendored, patched copy of upstream huggingface/speech-to-speech's own
// demo/main.js (see demo/Dockerfile's header comment for the vendoring
// convention). Local additions: the local_time/global_internet_search
// TOOL_DEFS entries, their branches in runTool(), and execMcpCall/
// execLocalTime/execGlobalSearch (speech-mcp/README.md) - everything else
// is unmodified upstream code.
/**
...
import { S2sRealtimeClient } from "./s2s-realtime-client.js?v=debug2";
...
const TOOL_DEFS = {
  ...
  local_time: {
    ...
    name: "local_time",
    description:
      "Get the current date and time for a given IANA timezone (e.g. 'Asia/Tokyo', " +
      "'America/New_York'). Use this whenever the user asks what time or date it is " +
      "somewhere, rather than guessing from training knowledge.",
    parameters: {
      type: "object",
      properties: {
        timezone: {
          type: "string",
          description:
            "IANA timezone name, e.g. 'America/New_York' or 'Asia/Tokyo'. Omit for the server's default.",
        },
      },
      required: [],
    },
  ...
  global_internet_search: {
    ...
    description:
      "Search the public internet for current or factual information you don't already " +
      "know (news, prices, facts, documentation). Returns concise results with titles, " +
      "snippets and URLs, via a self-hosted search engine.",
    parameters: {
      type: "object",
      properties: {
...
function activeToolDefs() {
  ...
  // MCP tools (speech-mcp/README.md): always on, no Settings toggle for this
  // PoC. In practice web_search above is usually unavailable (no Serper key
  // configured), so global_internet_search is the only search tool offered.
  defs.push(TOOL_DEFS.local_time, TOOL_DEFS.global_internet_search);
  return defs;
}
...
async function runTool(name, argsJson, callId) {
  ...
  try {
    ...
    } else if (name === "local_time") {
      const timezone = typeof args.timezone === "string" ? args.timezone : "";
      result.output = await execLocalTime(timezone);
    } else if (name === "global_internet_search") {
      const query = typeof args.query === "string" ? args.query : "";
      const maxResults = typeof args.max_results === "number" ? args.max_results : undefined;
      result.output = await execGlobalSearch(query, maxResults);
...
/**
 * Call one speech-mcp tool via the /api/mcp/call proxy (speech-mcp/README.md)
 * and parse its JSON result. Throws on transport/tool errors so callers can
 * surface them through runTool's existing catch.
 * @param {string} name @param {Record<string, unknown>} args @returns {Promise<unknown>}
 */
async function execMcpCall(name, args) {
...
/** @param {string} timezone @returns {Promise<string>} */
async function execLocalTime(timezone) {
  const data = await execMcpCall("local_time", timezone ? { timezone } : {});
  if (!data || typeof data !== "object") return String(data);
  const d = /** @type {Record<string, unknown>} */ (data);
  return `It is currently ${d.iso8601} (${d.weekday}) in ${d.timezone}.`;
}

/** @param {string} query @param {number} [maxResults] @returns {Promise<string>} */
async function execGlobalSearch(query, maxResults) {
  if (!query) return "No query provided.";
  const args = /** @type {Record<string, unknown>} */ ({ query });
  if (maxResults) args.max_results = maxResults;
  const data = await execMcpCall("global_internet_search", args);
  const results = Array.isArray(/** @type {any} */ (data)?.results) ? /** @type {any} */ (data).results : [];
  const today = new Date().toISOString().slice(0, 10);
  /** @type {string[]} */
  const lines = [`Internet search result from ${today}:`];
