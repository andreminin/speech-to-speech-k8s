...
# demo/server.py (see demo/Dockerfile's header comment for the vendoring
# convention). The only local addition is the /api/mcp/call proxy route
# near the bottom (MCP_SERVER_URL, McpCallRequest, mcp_call) - everything
# else is unmodified upstream code, most of which (LOAD_BALANCER_URL,
# limiter, auth) is inert in this deployment since SPEECH_TO_SPEECH_URL is
# always set (see k8s/demo/deployment.yaml).
"""
...
Endpoints:
  ...
  POST /api/mcp/call         -> { output }  proxies a tools/call to speech-mcp
...
# speech-mcp's Streamable HTTP endpoint (see speech-mcp/README.md). Same-origin
# proxy so the browser never needs the in-cluster service address, mirroring
# how /api/search holds the Serper key server-side.
MCP_SERVER_URL = os.environ.get("MCP_SERVER_URL", "http://speech-mcp:8080/mcp").strip()
...
async def mcp_call(req: McpCallRequest):
    """Proxy a single tools/call to speech-mcp (see speech-mcp/README.md).

    Generic and tool-agnostic: the caller (main.js) knows each tool's result
    shape and formats it for the model, this just relays the tool's text
    content (or raises the tool/JSON-RPC error) - same role /api/search plays
    for the Serper key, minus any response shaping."""
    if not MCP_SERVER_URL:
        raise HTTPException(status_code=503, detail="Tool server is not configured.")
    ...
    try:
        ...
        logger.warning("speech-mcp unreachable: %r", exc)
        raise HTTPException(status_code=502, detail="Tool server unreachable.")

    if resp.status_code != 200:
        body = resp.text[:300]
        logger.warning("speech-mcp error %s: %s", resp.status_code, body)
        raise HTTPException(status_code=502, detail=f"Tool server error ({resp.status_code}).")

    data = resp.json()
    rpc_error = data.get("error")
    if rpc_error:
        raise HTTPException(status_code=502, detail=rpc_error.get("message", "Tool call failed."))
    ...
    if result.get("isError"):
        raise HTTPException(status_code=502, detail=text or "Tool call failed.")
